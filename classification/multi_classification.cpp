...
string img_file = "./Model/img/";
string video_file = "./Model/video/";
char*  strLogDir = "./Model/log/";
...
void imgpro(Mat & src, frontal_face_detector &detect, shape_predictor &pose_models, std::vector<MultiClassifier> &all_classifiers, unsigned int &nLastCropWnd);
bool get_filelist_from_dir(string &path, std::vector<string>& files);
...
int main(int argc, char** argv)
{
	...
	std::vector<MultiClassifier> all_classifier;
	printf("Loading Gender recog training data ..... ");
	MultiClassifier classifier_gender(model_file, trained_file, mean_file, label_file);
	all_classifier.push_back(classifier_gender);
	printf("Done!\n");
	printf("Loading Age recog training data ..... ");
	MultiClassifier classifier_age(model_file_age, trained_file_age, mean_file, label_file_age);
	all_classifier.push_back(classifier_age);
	printf("Done!\n");
	printf("Loading Age recog training data ..... ");
	MultiClassifier classifier_emo(model_file_emo, trained_file_emo, mean_file_emo, label_file_emo);
	all_classifier.push_back(classifier_emo);
	printf("Done!\n");
	//载入人脸识别训练数据
	...
	case 1:
	{
			  ...
			  for (int i = 0; i < file_list.size(); i++)
			  {
				  ...
				  //string image_path = file_list[i];
				  Mat image = imread(image_path);
				  imgpro(image, detector, pose_model, all_classifier, nLastCropWnds);
	...
	case 2:
	{
			  printf("Initialize camera and do recog .... \n");
			  int camindex = 0;
			  ...
			  while (capture.isOpened())
			  {
				  ...
				  imgpro(I, detector, pose_model, all_classifier, nLastCropWnds);
				  //-----------------------------------------------------------------------------	
			  }
			  break;
	}
	case 3:
	{
			  string file_path = video_file;
			  string search_path = file_path + "*.avi";
			  std::vector<string> file_list;
			  if (!get_filelist_from_dir(search_path, file_list))
				  cout << "open file error!" << endl;
			  for (int i = 0; i < file_list.size(); i++)
...
{
	Mat I = src;
	frontal_face_detector detector = detect;
	shape_predictor pose_model = pose_models;
	int nLastCropWnds = nLastCropWnd;

	MultiClassifier classifier_genders = all_classifiers[0];
	MultiClassifier classifier_ages = all_classifiers[1];
	MultiClassifier classifier_emos = all_classifiers[2];
	//MultiClassifier classifier_faces = all_classifiers[3];
	//-----------------------------------------------------
	//人脸检测及面部特征点定位
	...
	for (unsigned int i = 0; i < vecFaceRects.size(); i++)
	{
		...
		rec_crop.x = cvRound(pt_eye_right.x - 0);	rec_crop.y = cvRound(pt_eye_right.y - 0);
	...
	for (unsigned int i = 0; i < vecIcrop.size(); i++)
	{
		...
		std::vector<Prediction> predictions_gender = classifier_genders.Classify(I_crop, 1);
		t1 = (double)cvGetTickCount() - t1;
		//年龄识别
		double t2 = (double)cvGetTickCount();
		std::vector<Prediction> predictions_age = classifier_ages.Classify(I_crop, 8);
		t2 = (double)cvGetTickCount() - t2;
		//表情识别
		double t3 = (double)cvGetTickCount();
		std::vector<Prediction> predictions_emo = classifier_emos.Classify(I_crop, 10);
...
//		sprintf(strTempFileFind, "%s", path);
//
//		BOOL IsFinded = (BOOL)tempFind.FindFile(strTempFileFind);
//		while (IsFinded)
//		{
...
//					tmpfilepath = tempFind.GetFilePath();
//					files.push_back(tmpfilepath);
//				}
//			}
//		}
//		tempFind.Close();
